...
class Spider:
    def __init__(self):
        ...
        # 设置用户代理，伪装成浏览器
        self.ua_header = {"User-Agent" : "Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/75.0.3770.100 Safari/537.36"}
        # 设置图片编号
        self.userName = 1
    ...
    def writeImages(self, imagesLink):
        '''
            将 images 里的二进制内容存入到userNname文件中
        '''
        print imagesLink
        print "正在存储第%d张图片 ..." % self.userName
        # 1. 打开文件，返回一个文件对象
        file = open(r'C:\Users\hp\Desktop\doucement\images\picture' + str(self.userName)  + '.png', 'wb')
        # 2. 获取图片里的内容
        content = urllib2.urlopen(imagesLink).read()
